...
foreach ($formFields as $field => $option) {
    ...
    if ($option['type'] == 'dropdown' || $option['type'] == 'textinput-dropdown') {
        ...
        $jscript .= '
        content.find("#" + inputId' . $field . ').select2({
            theme: "krajee",
            placeholder: "Pilih",
            allowClear: true
        });
        ...
        if (!empty($option['affect'])) {
            ...
            $jscript .= '
            var inputIdAffect' . $affectField . ' = content.find("#' . strtolower(Inflector::camelize($model->tableName())) . '-index-' . $affectField .'").attr("id");
            ...
            $.ajax({
                dataType: "json",
                cache: false,
                url: "' . $option['affect']['url'] . '?id=" + content.find("#" + inputId' . $field . ' + " option:selected").val(),
                success: function(response) {
                    dropdownFunction' . $affectField . '(response);
                }
            });

            content.find("#" + inputId' . $field . ').on("select2:select", function(e) {
            ...
            content.find("#" + inputId' . $field . ').on("select2:unselect", function(e) {
        ...
        $jscript .= '
            var inputIdDisp' . $field . ' = content.find("#' . strtolower(Inflector::camelize($model->tableName())) . '-index-' . $field .'-disp").attr("id");
            inputIdDisp' . $field . ' = inputIdDisp' . $field . '.replace("index", index);
            
            content.find("#' . strtolower(Inflector::camelize($model->tableName())) . '-index-' . $field .'-disp").attr("id", inputIdDisp' . $field . ');

            content.find("#" + inputId' . $field . ' + "-disp").maskMoney(maskMoney_setting);

            var val = parseFloat(content.find("#" + inputId' . $field . ').val());
            content.find("#" + inputId' . $field . ' + "-disp").maskMoney("mask", val);
            content.find("#" + inputId' . $field . ' + "-disp").on("change keyup", function (e) {                
                if (e.type ==="change" || (e.type === "keyup" && (e.keyCode == 13 || e.which == 13))) {
                    var out = content.find("#" + inputId' . $field . ' + "-disp").maskMoney("unmasked")[0];
                    content.find("#" + inputId' . $field . ').val(out).trigger("change");                        
                }
            });
        ';
    }
...
$jscript .= '
        ...
        $("#tbody-' . $model->tableName() . '").append(content);
