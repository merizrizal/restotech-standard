...
foreach ($modelSaleInvoice as $dataSaleInvoice) {
    ...
    foreach ($dataSaleInvoice['saleInvoiceTrxes'] as $dataSaleInvoiceTrx) {
        ...
        foreach ($dataSaleInvoiceTrx['menu']['menuHpps'] as $dataMenuHpp) {

            $subtotalHpp = $dataMenuHpp['harga_pokok'] * $dataSaleInvoiceTrx['jumlah'];
        }

        if (!empty($dataMenu[$keyMenu]['subtotalHpp'])) {
            $dataMenu[$keyMenu]['subtotalHpp'] += $subtotalHpp;
        } else {
            $dataMenu[$keyMenu]['subtotalHpp'] = $subtotalHpp;
        }
        ...
        $jumlahTotalHpp += $subtotalHpp;
        $jumlahTotalSelisih += $subtotal - $subtotalHpp;
            ...
            <table class="table">
                <thead>
                    <tr style="border:1px solid">
                        ...
                        <th style="width: 180px" class="number">Subtotal HPP</th>
                        <th style="width: 180px" class="number">Selisih</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    foreach ($dataMenu as $value):
                        ...
                        <tr>
                            ...
                            <td class="line number"><?= Tools::convertToCurrency($value['subtotalHpp'], ($print == 'pdf')) ?></td>
                            <td class="line number"><?= Tools::convertToCurrency($value['subtotal'] - $value['subtotalHpp'], ($print == 'pdf')) ?></td>
                        </tr>
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>Total Hpp</td>
                                        <td>:</td>
                                        <td class="number"><?= Tools::convertToCurrency($jumlahTotalHpp, ($print == 'pdf')) ?></td>
                                    </tr>
                                    <tr>
                                        <td>Total Selisih</td>
                                        <td>:</td>
                                        <td class="number"><?= Tools::convertToCurrency($jumlahTotalSelisih, ($print == 'pdf')) ?></td>
                                    </tr>
                                </tbody>
                            </table>
