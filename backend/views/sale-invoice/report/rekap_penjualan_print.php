...
if (!empty($modelSaleInvoice)):
    ...
    foreach ($modelSaleInvoice as $dataSaleInvoice) {
        ...
        foreach ($dataSaleInvoice['saleInvoiceTrxes'] as $dataSaleInvoiceTrx) {                        
            ...
            $keyMenu = $kategoriParent ? $dataSaleInvoiceTrx['menu']['menuCategory']['parentCategory']['id'] : $dataSaleInvoiceTrx['menu']['menuCategory']['id'];
            $keyMenu2 = $dataSaleInvoiceTrx['menu']['id'];

            $dataMenu[$keyMenu]['namaCategory'] = $kategoriParent ? $dataSaleInvoiceTrx['menu']['menuCategory']['parentCategory']['nama_category'] : $dataSaleInvoiceTrx['menu']['menuCategory']['nama_category'];
    ...
    $dataMenu = Tools::array_sort($dataMenu, 'namaCategory'); ?>
        ...
        <div class="col-lg-12">
                    ...
                    foreach ($dataMenu as $keyC => $menuCategory):
                        ...
                        foreach ($menuCategory as $key => $menu) { 
                            ...
                            if ($key != 'namaCategory') {
                                ...
                                $totalQty += $menu['qty'];
                                $totalHarga += $menu['subtotal'];
                            }
                        } ?>
                    
                        <tr>
                            <td><?= $menuCategory['namaCategory'] ?></td>
                            <td><?= $totalQty?></td>
                            <td class="number"><?= Tools::convertToCurrency($totalHarga, ($print == 'pdf')) ?></td>
                        </tr>
            ...
            <table class="table">
                <tbody>
                    <tr>
                        ...
                        <th class="number" style="width: 200px">Jumlah</th>    
                    </tr>
            ...
            <table class="table">
                <tbody>
                    ...
                    <tr>
                        <td>GRAND TOTAL</td>
                        <td class="number"><?= Tools::convertToCurrency(($cashPayment - ($jumlahKembalian + $jumlahRefund + $jumlahRefundServiceCharge + $jumlahRefundPajak)), ($print == 'pdf')) ?></td>                      
                    </tr>
                </tbody>
            </table>
        </div>
