...
if (count($modelMenu) > 0):
    ...
    echo Html::hiddenInput('order_parent_id', $orderParentId, ['id' => 'order-parent-id']);
    
    foreach ($modelMenu as $menuData):
        
        $badgeStock = '';
    
        if (count($menuData['menu']['menuRecipes']) > 0) {         
            
            $noStock = '<div class="badge badge-hot"><i class="ion ion-minus" style="font-size: 48px; margin-top: 8px"></i></div>';
            
            foreach ($menuData['menu']['menuRecipes'] as $dataMenuRecipe) {
        ...
        <a href="#" id="menu">
            <div class="col-md-3 col-sm-3 mb">
                <div class="product-panel-2 pn" style="padding: 10px 0">
                    ...
                    <img src="<?= Yii::getAlias('@uploadsUrl') . Tools::thumb('/img/menu/', (!empty($menuData['menu']['image']) ? $menuData['menu']['image'] : 'noimage.png'), 120, 120) ?>" width="120" class="img-circle" style="margin: 10px 0 10px 0">
                    ...
                    <?= Html::hiddenInput('menu_id', $menuData['menu']['id'], ['class' => 'menu-id menu']) ?>
                    <?= Html::hiddenInput('nama_menu', $menuData['menu']['nama_menu'], ['class' => 'nama-menu menu']) ?>
                    <?= Html::hiddenInput('harga_jual', $menuData['menu']['harga_jual'], ['class' => 'harga-jual menu']) ?>
                    
                    <?php                    
                    if (!empty($menuData['menu']['menuCategory']['menuCategoryPrinters'])) {
                        
                        foreach ($menuData['menu']['menuCategory']['menuCategoryPrinters'] as $value) {
    ...
    $("a#menu").on("click", function(event) {
        ...
        $.ajax({
            ...
            data: {
                ...
                "parent_id": $("#order-parent-id").val(),
                "menu_id": menuId,                
                "harga_satuan": harga,
            },
            url: "' . Yii::$app->urlManager->createUrl(["action/save-order"]) . '",
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    var comp = $("#temp-order").children().clone();

                    comp.find(".order-id.order").val(response.order_id);
                    comp.find(".parent-id.order").val($("#order-parent-id").val());
                    ...
                    thisObj.find(".printer.menu").each(function() {
                        ...
                        comp.find("#menu-row").append(printer.clone());
                    });

                    comp.find("#no").html("<i class=\"fa fa-plus\" style=\"color:green\"></i> ");
                    ...
                    $("tbody#order-menu tr#menu-row.highlight").after(comp.html());  
                } else {
